...
能跟你聊天、能寫文章、能寫程式碼的那種 AI。本書尹知行前世吃飯的工具、藏經閣裡所有「會判斷」的判官、童子的本體。
...
2022 年的論文標題。Agent 的經典模式：每一步先「想」（Thought）、再「做」（Action）、看結果、再想。本書第 9 章尹知行教阿土的「想 → 做 → 看 → 想」就是 ReAct。
...
最基本的 fine-tune 方式：給「問題 → 標準答案」的配對，逼模型學會。本書尹知行給判官讀的 500 條圈案。
...
**▍Demo**（展示／樣品）
給客戶或老闆看的功能展示。本書第 1 章尹知行被罵的那場就是 demo 翻車。
...
不同類型問題走不同處理路徑：簡單問題用便宜模型、複雜問題用貴的。本書第 13 章尹知行對阿土說的策略。
...
**▍Decision Log**（決策日誌）
記下「為什麼選 A 不選 B、什麼時候改的、改的原因」的文件。本書第 17 章尹知行交付給後人的東西之一。

**▍MVP**（Minimum Viable Product，最小可行產品）
能跑、能驗證假設、最小的版本。本書「先 ship 再迭代」的精神（但尹知行第 1 章吃了這個的虧）。
...
**這份附錄會持續更新**。如果你讀正文遇到任何沒解釋到的英文詞、或解釋得不夠清楚的地方，告訴尹知行，下一版補進來。
